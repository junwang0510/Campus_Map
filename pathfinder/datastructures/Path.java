...
/**
 * This represents an immutable path between two nodes of data type N, particularly
...
public class Path<N> implements Iterable<Path<N>.Segment> {
    ...
    //          path.get(0) is the first step from the starting node to an intermediate node and
    //          path.get(path.size() - 1) is the final step from an intermediate node to the end
    //      total cost along the path => cost
    //      the destination node in this path, opposite the starting node => getEnd()
    ...
    /**
     * Creates a new, empty path containing a starting node. Essentially this represents a path
     * from the starting node to itself with a total cost of "0".
     *
    ...
    /**
     * @return The node at the end of this path, which may be the starting node if this path
     * contains no segments (i.e. this path is from the starting node to itself).
     */
    public N getEnd() {
        if(path.size() == 0) {
            return start;
     ...
     * the starting node and itself, this iterator contains no elements. This iterator does not
